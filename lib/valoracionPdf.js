...
export function generarPdfValoracion({ folio, sujeto = {}, parametros = {}, resultado = {}, logoDataUrl = null }) {
  ...
  const linea2 = `Rango ${f0(est.rango_uf?.[0])} – ${f0(est.rango_uf?.[1])} UF   ·   ${f2(est.uf_m2_mediana)} UF/m²`
    + (est.valor_clp ? `   ·   ≈ $${f0(est.valor_clp)}` : '')
    + (resultado.vs_avaluo ? `   ·   ${f2(resultado.vs_avaluo.ratio)}× avalúo fiscal` : '');
