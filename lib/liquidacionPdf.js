...
export async function generarPdfLiquidacion({ bloque, mesTxt, fecha, despedida, logoDataUrl, borrador }) {
  ...
  // Logo opcional (PNG/JPG en dataURL). Si no hay, cabecera de texto.
  let logoImg = null
  if (logoDataUrl) {
    try {
      const b64 = logoDataUrl.split(',')[1]
      const bytes = Uint8Array.from(Buffer.from(b64, 'base64'))
      logoImg = logoDataUrl.includes('image/png')
        ? await pdf.embedPng(bytes)
        : await pdf.embedJpg(bytes)
    } catch { logoImg = null }
  }

  const text = (t, x, yy, f = font, size = 9, color = DARK) => page.drawText(String(t ?? ''), { x, y: yy, size, font: f, color })
  const rightText = (t, xRight, yy, f = font, size = 9, color = DARK) => {
    const s = String(t ?? ''); text(s, xRight - f.widthOfTextAtSize(s, size), yy, f, size, color)
  }
