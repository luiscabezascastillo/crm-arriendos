...
const ESPERA = rgb(0.114, 0.306, 0.847)   // azul: importe en espera de cobro
const ESPERA_GREY = rgb(0.72, 0.74, 0.79)  // atenuado (difuminado intenso): "A transferir" de una línea en espera (no se suma)
...
export async function generarPdfLiquidacion({ bloque, mesTxt, fecha, despedida, logoDataUrl, borrador, factorEscala }) {
  ...
  for (const x of (bloque.inmuebles || [])) {
    ...
    for (const c of cols) {
      ...
      let color = DARK
      if (c.key === 'descuentos' && x.descuentos) color = GREEN
      else if (c.key === 'aTransferir' && x.notaEspera) color = ESPERA_GREY   // en espera: atenuado (no se suma)
    ...
    if (!x.esP) {
      ...
      // "En espera": el arrendatario no ha pagado → su transferencia se aplaza hasta el cobro.
      if (x.notaEspera) subs.push({ color: rgb(0.11, 0.31, 0.85), monto: '', txt: x.notaEspera })
    }
  ...
  // "A transferir" del total = importe REAL de esta oleada (sin lo que está en espera de cobro).
  const enEspera = Math.round(n0(t.enEspera))
  const ahora = (t.aTransferirAhora != null) ? Math.round(n0(t.aTransferirAhora)) : Math.round(n0(t.aTransferir))
  put('aTransferir', enEspera > 0 ? ahora : t.aTransferir)
  y -= (enEspera > 0 ? 16 : 30) * esc
  // Línea "En espera de cobro" (solo si hay algún inmueble retenido): el importe aplazado y el motivo.
  if (enEspera > 0) {
    text('En espera de cobro:', M + 12, y - 10 * esc, bold, 8.5 * esc, ESPERA)
    rightText(money(enEspera), M + 210 * (contentW / 762), y - 10 * esc, bold, 8.5 * esc, ESPERA)
    text('se transferira al conseguir el cobro (el arrendatario aun no ha pagado)', M + 220 * (contentW / 762), y - 10 * esc, font, 8 * esc, ESPERA)
    y -= 22 * esc
  }
