...
// Teléfono chileno -> { valido, e164, wa (dígitos), esMovil, display }.
export function normalizaTelefono(raw) {
  let n = soloDigitos(raw)
  if (!n) return { valido: false }
  if (n.startsWith('00')) n = n.slice(2)
  if (n.length === 11 && n.startsWith('56')) { /* ok: 56 + 9 dígitos */ }
  else if (n.length === 9 && n.startsWith('9')) n = '56' + n            // 9XXXXXXXX -> 569XXXXXXXX
  else if (n.length === 8) n = '569' + n                                 // móvil sin prefijo
  else if (n.length === 10 && n.startsWith('56')) n = '569' + n.slice(2) // 56XXXXXXXX (falta el 9)
  else if (n.length === 9 && !n.startsWith('9')) n = '56' + n
  else if (!n.startsWith('56')) return { valido: false }
  const esMovil = n.length === 11 && n.startsWith('569')
  return {
    valido: true, e164: '+' + n, wa: n, esMovil,
