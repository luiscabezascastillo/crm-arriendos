...
// Logo: sube el PNG (blanco, fondo transparente) a public/logo-fcr.png en el
// proyecto. Quedará accesible en esta URL. Si la imagen no carga, se muestra
// el texto "FONDO CAPITAL" como respaldo.
const LOGO_URL = 'https://crm-arriendos.vercel.app/logo-fcr.png'

export const fmt = (n) => Math.round(Number(n) || 0).toLocaleString('es-CL')

// Escapa el texto libre del usuario para insertarlo con seguridad en el HTML del correo.
...
// ctx: { mes, mesLabel, valorUf, notaCorreccion }
// notaCorreccion (opcional): texto que se muestra destacado al principio del correo,
//   para reenvíos correctores (p. ej. cuando se corrige el tipo de ajuste de un contrato).
export function plantillaArriendo(n, ctx) {
  const M = (ctx.mesLabel || '').toUpperCase()
  const uf = esUF(n.revision)

  const nota = (ctx.notaCorreccion || '').trim()
  const bloqueNota = nota
    ? `<div style="background:#FEF3C7;border:1px solid #FCD34D;border-radius:8px;padding:14px 18px;margin:0 0 18px;">
         <div style="font-size:12px;font-weight:700;color:#92400E;margin-bottom:6px;text-transform:uppercase;letter-spacing:.03em;">Comunicación correctora</div>
         <div style="font-size:14px;color:#78350F;line-height:1.6;white-space:pre-wrap;">${escaparHtml(nota)}</div>
       </div>`
    : ''
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
          <tr>
            <td style="vertical-align:middle;padding-right:16px;">
              <img src="${LOGO_URL}" alt="Fondo Capital" height="46" style="display:block;height:46px;border:0;outline:none;text-decoration:none;" />
            </td>
            <td style="vertical-align:middle;border-left:2px solid #D1D5DB;padding-left:16px;">
              <span style="color:#1f2937;font-size:15px;font-weight:600;letter-spacing:.02em;">Administración de Propiedades</span>
            </td>
          </tr>
        </table>
      </div>
