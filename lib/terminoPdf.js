...
export async function generarPdfTermino({ datos, fecha, borrador, logoDataUrl }) {
  ...
  // El PDF NO muestra el propietario, y en la garantía solo el importe (sin "quién la tiene").
  const metaL = [['IDADMON', d.idadmon || ''], ['Inmueble', d.inmueble || ''], ['Fecha de entrega', d.fechaEntrega || '']]
  const metaR = [['Arrendatario', d.arrendatario || ''], ['Garantia', money(d.garantia?.monto)]]
