...
export async function generarPdfTermino({ datos, fecha, borrador, logoDataUrl }) {
  ...
  const metaR = [['Arrendatario', d.arrendatario || ''], ['Propietario', d.propietario || ''], ['Garantia / quien la tiene', `${money(d.garantia?.monto)}  (${d.garantia?.quien || '-'})`]]
  for (let i = 0; i < 3; i++) {
    text(metaL[i][0] + ':', M, y - 10, bold, 8.5, GREY); text(fit(metaL[i][1], font, 9.5, colW - 100), M + 96, y - 10, font, 9.5, DARK)
    text(metaR[i][0] + ':', M + colW, y - 10, bold, 8.5, GREY); text(fit(metaR[i][1], font, 9.5, colW - 105), M + colW + 105, y - 10, font, 9.5, DARK)
    y -= 15
  }
  y -= 8
  ...
  if (has(repDet) || has(repLin)) {
    sectionTitle('REPARACIONES Y PRESUPUESTO APLICADO')
    if (has(repDet)) {
      tableRows(
        [{ label: 'Presupuesto - detalle', w: 420, align: 'l' }, { label: 'Importe', w: 90, align: 'r' }],
        repDet.map(x => [x.descripcion, money(x.importe)]),
        { total: rep.presupuesto?.total, totalLabel: 'Total presupuesto (precio final, IVA incluido)' }
      )
    }
    if (has(repLin)) {
      tableRows(
        [{ label: 'Otras reparaciones', w: 190, align: 'l' }, { label: 'Comentario', w: 230, align: 'l' }, { label: 'Importe', w: 90, align: 'r' }],
        repLin.map(x => [x.concepto, x.comentario || '', money(x.monto)])
      )
    }
    y -= 6
  }
  ...
  if (has(d.descuentos)) {
    sectionTitle('DESCUENTOS APLICADOS (cercanos al cierre)')
    tableRows(
  ...
  page.drawRectangle({ x: M, y: y - 48, width: contentW, height: 48, color: BOXBG, borderColor: LINE, borderWidth: 0.6 })
  text('RESULTADO DEL TERMINO', M + 12, y - 17, bold, 10, DARK)
  text(`Estado: ${res.tipo || ''}`, M + 12, y - 33, font, 9.5, GREY)
  rightText(money(res.valor), M + contentW - 14, y - 22, bold, 20, resColor)
  rightText(res.label || '', M + contentW - 14, y - 38, font, 9, GREY)
  y -= 58
