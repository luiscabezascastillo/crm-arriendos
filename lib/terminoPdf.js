...
export async function generarPdfTermino({ datos, fecha, borrador, logoDataUrl }) {
  ...
  if (has(repDet) || repTotal > 0 || has(repLin)) {
    ...
    // Tabla única y plana: el detalle del presupuesto, luego UNA línea "Arreglos segun presupuesto" (sin negrita ni
    // total destacado, sin repetir en un bloque aparte) y, si las hubiera, otras reparaciones. Nada se duplica.
    const rows = []
    for (const x of repDet) rows.push([x.descripcion, money(x.importe)])
    if (repTotal > 0) rows.push(['Arreglos segun presupuesto', money(repTotal)])
