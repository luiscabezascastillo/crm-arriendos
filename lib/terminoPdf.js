// lib/terminoPdf.js
// VERSION: v9 · 2026-08-12 · Solo TEXTO en el bloque del propietario: "NO REPERCUTIDO" pasa a "REPERCUTIDO"
//   (es al revés: SÍ se le ha descontado al propietario; se reclama al ex-arrendatario/aval y se le abona si se
//   recupera). Números, negativos y pie (liquidación real · en reclamación · ASUMIDO POR EL PROPIETARIO) SIN
//   cambios respecto a v8. Hereda v8.
// VERSION: v8 · 2026-08-12 · PDF del PROPIETARIO con conceptos en reclamación: si datos.reclamacion llega,
//   se pinta un bloque naranja con las líneas y el RESULTADO pasa a tres cifras. El PDF del arrendatario
//   (sin datos.reclamacion) no cambia. Hereda v7.
//   datos.reclamacion = { lineas:[{concepto,comentario,monto}], resultadoReal:{valor,label},
//     resultadoAsumido:{valor}, pendiente }   // pendiente = real - asumido
// VERSION: v7 · 2026-08-11 · (1) Línea de GARANTÍA ENTREGADA con FONDO NEGRO (texto blanco, importe en verde claro).
//   (2) Se RETIRA la sección "Descuentos aplicados": repetía lo que ya sale en Servicios/Reparaciones. Hereda v6.
...
const ORANGE = rgb(0.72, 0.33, 0.05)   // "no repercutido / en reclamación" (líneas 🚫 del propietario)
const ORANGEBG = rgb(0.99, 0.95, 0.90)
...
export async function generarPdfTermino({ datos, fecha, borrador, logoDataUrl }) {
  ...
  if (n0(d.garantia && d.garantia.monto) > 0) {
    ...
    page.drawRectangle({ x: M, y: y - 18, width: contentW, height: 18, color: rgb(0.10, 0.10, 0.12) })
    text('GARANTIA ENTREGADA', M + 8, y - 12.5, bold, 9.5, rgb(1, 1, 1))
    rightText(money(d.garantia.monto), M + contentW - 8, y - 12.5, bold, 10.5, rgb(0.34, 0.82, 0.50))
    y -= 24
  }
  ...
  // (La sección "DESCUENTOS APLICADOS" se retira: repetía lo que ya figura en Servicios/Reparaciones.)

  // ── NO REPERCUTIDO AL PROPIETARIO (en reclamación al ex-arrendatario / aval) ──
  // Solo llega en el PDF del propietario cuando hay líneas 🚫. Se muestran en NEGATIVO y en naranja:
  // la liquidación de arriba es "lo que asume el propietario"; esto es lo que NO se le repercute y se reclama.
  const rec = d.reclamacion
  if (rec && has(rec.lineas)) {
    // SÍ se le ha descontado al propietario (lo asume); se reclama al ex-arrendatario/aval y se le abona si
    // se recupera. Por eso el rótulo es "REPERCUTIDO" (no "no repercutido"). Resto del bloque igual que v8.
    sectionTitle('REPERCUTIDO AL PROPIETARIO - EN RECLAMACION AL EX-ARRENDATARIO / AVAL')
    ensure(16)
    text('Concepto', M + 4, y - 11, bold, 8, GREY)
    text('Comentario', M + 210, y - 11, bold, 8, GREY)
    rightText('Repercutido', M + contentW - 4, y - 11, bold, 8, GREY)
    page.drawLine({ start: { x: M, y: y - 14 }, end: { x: M + contentW, y: y - 14 }, thickness: 0.6, color: LINE }); y -= 15
    let sumEx = 0
    for (const x of rec.lineas) {
      ensure(14)
      const m = n0(x.monto); sumEx += m
      text(fit(x.concepto || '', font, 8.5, 200), M + 4, y - 10, font, 8.5, DARK)
      text(fit(x.comentario || '', font, 8.5, 200), M + 210, y - 10, font, 8.5, GREY)
      rightText('-' + money(m), M + contentW - 4, y - 10, font, 8.5, ORANGE)
      page.drawLine({ start: { x: M, y: y - 13 }, end: { x: M + contentW, y: y - 13 }, thickness: 0.4, color: LINE }); y -= 14
    }
    ensure(17)
    page.drawRectangle({ x: M, y: y - 15, width: contentW, height: 15, color: ORANGEBG })
    text('Total repercutido (a recuperar del arrendatario / aval)', M + 6, y - 11, bold, 8.5, ORANGE)
    rightText('-' + money(sumEx), M + contentW - 4, y - 11, bold, 8.5, ORANGE); y -= 17
    y -= 6
  }
  ...
  if (rec && has(rec.lineas)) {
    // Tres cifras (igual que v8): liquidación real (todos los conceptos) · en reclamación (a recuperar) ·
    // ASUMIDO por el propietario (esta liquidación).
    ensure(86)
    const boxH = 74
    page.drawRectangle({ x: M, y: y - boxH, width: contentW, height: boxH, color: BOXBG, borderColor: LINE, borderWidth: 0.6 })
    const vReal = n0(rec.resultadoReal && rec.resultadoReal.valor)
    const pend = n0(rec.pendiente)
    text('Liquidacion real del termino (todos los conceptos)', M + 12, y - 18, font, 9.5, GREY)
    rightText(money(vReal), M + contentW - 14, y - 18, bold, 11, DARK)
    text('En reclamacion al ex-arrendatario / aval (a recuperar)', M + 12, y - 34, font, 9.5, ORANGE)
    rightText((pend > 0 ? '-' : '+') + money(Math.abs(pend)), M + contentW - 14, y - 34, bold, 11, ORANGE)
    page.drawLine({ start: { x: M + 12, y: y - 43 }, end: { x: M + contentW - 12, y: y - 43 }, thickness: 0.6, color: LINE })
    text('ASUMIDO POR EL PROPIETARIO (esta liquidacion)', M + 12, y - 61, bold, 10.5, DARK)
    rightText(money(res.valor), M + contentW - 14, y - 61, bold, 18, resColor)
    y -= boxH + 8
    ensure(14)
    text('La diferencia se reclama al ex-arrendatario y/o al aval; cualquier recuperacion se abonara al propietario.', M, y - 2, font, 8, GREY)
    y -= 14
  } else {
