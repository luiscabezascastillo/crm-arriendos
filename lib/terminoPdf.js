// lib/terminoPdf.js
// VERSION: v8 · 2026-08-12 · PDF del PROPIETARIO con conceptos NO REPERCUTIDOS: si datos.reclamacion llega,
//   se pinta un bloque naranja con las líneas 🚫 en NEGATIVO ("en reclamación al arrendatario/aval") y el
//   RESULTADO pasa a tres cifras: liquidación real · en reclamación · asumido por el propietario. El PDF del
//   arrendatario (sin datos.reclamacion) no cambia. Hereda v7.
...
export async function generarPdfTermino({ datos, fecha, borrador, logoDataUrl }) {
  ...
  if (rec && has(rec.lineas)) {
    sectionTitle('NO REPERCUTIDO AL PROPIETARIO - EN RECLAMACION AL EX-ARRENDATARIO / AVAL')
    ensure(16)
    text('Concepto', M + 4, y - 11, bold, 8, GREY)
    text('Comentario', M + 210, y - 11, bold, 8, GREY)
    rightText('No repercutido', M + contentW - 4, y - 11, bold, 8, GREY)
    page.drawLine({ start: { x: M, y: y - 14 }, end: { x: M + contentW, y: y - 14 }, thickness: 0.6, color: LINE }); y -= 15
    let sumEx = 0
    ...
    text('Total no repercutido (a recuperar del arrendatario / aval)', M + 6, y - 11, bold, 8.5, ORANGE)
  ...
  if (rec && has(rec.lineas)) {
    // Tres cifras: liquidación REAL (todas las líneas) · lo NO repercutido (en reclamación) · lo ASUMIDO por el propietario.
    ensure(86)
    const boxH = 74
