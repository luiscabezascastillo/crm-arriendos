// lib/cc1Permisos.js
// VERSION: 2026-08-14 · +AUDITORIA_TERMINO y capacidad puedeAuditarTermino (Dirección + Karina) para los
//   sub-estados Q-Auditado / Q-Reclamado del término. El resto de capacidades, sin cambios.
...
// Auditoría / reclamación del término (Q → Q-Auditado / Q-Reclamado): acción sensible,
// SOLO Dirección + Karina (misma lista que las acciones sensibles de liquidación/facturación).
export const AUDITORIA_TERMINO = [
  'luis.cabezas@fondocapital.com',
  'alberto.cabezas@fondocapital.com',
  'karina.morales@fondocapital.com',
]

// Devuelve { rol, esDireccion, puedeCambiarEstado, puedeAltaDirecta, puedeAltaPendiente, puedeAprobar, puedeCorregirTermino }
export async function getCapacidades(email) {
  ...
  return {
    ...
    // Corregir SOLO la fecha de termino_actual en un contrato activo (por si la fecha de salida
    // se equivocó o el arrendatario tardó en salir). Supervisor (Administración) y responsable.
    // NO desbloquea el resto del LOG: solo ese campo. Queda registrado con motivo.
    puedeCorregirTermino: esResponsable || rol === 'supervisor',
    // Reasignar la composición del inmueble (elegir otra combinación de idinmue existente para el
    // contrato). Solo Dirección y Legal (Anthony): es una reagrupación excepcional.
    puedeComponerInmueble: esResponsable || e === 'anthony.mendoza@fondocapital.com',
    // Crear/editar inmuebles y agrupaciones (vista Inmuebles). Administración (supervisor),
    // Dirección y Legal (Anthony). Quien solo consulta no es supervisor → no puede.
    puedeEditarInmuebles: esResponsable || rol === 'supervisor' || e === 'anthony.mendoza@fondocapital.com',
    ...
    // Marcar un término como AUDITADO o RECLAMADO (Q → Q-Auditado / Q-Reclamado): acción
    // sensible, SOLO Dirección + Karina (independiente del rol en revision_log).
    puedeAuditarTermino: AUDITORIA_TERMINO.includes(e),
  }
}
