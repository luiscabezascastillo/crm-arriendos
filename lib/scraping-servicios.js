...
export async function consultarEnel(codigo) {
  ...
  try {
    ...
    // Servipag es una SPA (Angular): esperar a que el formulario se renderice.
    // Esperamos explícitamente a que exista un input de texto visible.
    try {
      await page.waitForFunction(() => {
        return Array.from(document.querySelectorAll('input')).some(i => i.offsetParent !== null)
      }, { timeout: 20000 })
    } catch (e) { /* seguimos; el diagnóstico de abajo dirá qué hay */ }
    await new Promise(r => setTimeout(r, 2000))

    // Localizar el campo del número de cliente; si no, devolver diagnóstico
    const infoCampo = await page.evaluate(() => {
      const inputs = Array.from(document.querySelectorAll('input'))
      const visibles = inputs.filter(i => i.offsetParent !== null)
      const elegido = visibles.find(i => {
        const ph = (i.placeholder || '').toLowerCase()
        return ph.includes('cliente') || ph.includes('identific') || ph.includes('número') || ph.includes('numero')
      }) || visibles.find(i => i.type === 'text' || !i.type) || visibles[0]
      if (elegido) {
        elegido.setAttribute('data-fcr-target', '1')  // marcamos para escribir luego
        return { ok: true }
      }
      // diagnóstico: qué inputs hay
      return {
        ok: false,
        totalInputs: inputs.length,
        visibles: visibles.length,
        detalle: inputs.slice(0, 8).map(i => ({ type: i.type, ph: i.placeholder, id: i.id, name: i.name, vis: i.offsetParent !== null })),
      }
    })

    if (!infoCampo.ok) {
      await browser.close()
      return { error: 'No se encontró el campo de número de cliente', textoDebug: JSON.stringify(infoCampo).slice(0, 400) }
    }

    // Escribir el número en el campo marcado
    await page.focus('[data-fcr-target="1"]').catch(() => {})
    await page.evaluate(() => { const el = document.querySelector('[data-fcr-target="1"]'); if (el) el.value = '' })
    await page.type('[data-fcr-target="1"]', identifier, { delay: 60 }).catch(() => {})
