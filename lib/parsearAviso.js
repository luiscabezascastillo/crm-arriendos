...
function extraerM2(txt, htmlRaw) {
  ...
  let v = jsonAttr('COVERED_AREA'); if (v) return { m2: v, tipo: 'util(json)' };
...
// LISTADO: capta avisos en UF y en pesos ($)
export function parsearListado(texto) {
  const lineas = String(texto || '').split('\n').map((l) => l.trim()).filter(Boolean);
  const items = [];
  for (let i = 0; i < lineas.length; i++) {
    const linea = lineas[i];
    // detecta "UF" sola, "$" solo, o "$ 53.000.000" / "UF 1.650" en una linea
    let moneda = null, precio = null;
    if (/^UF$/i.test(linea)) { moneda = 'UF'; precio = toNum(lineas[i + 1]); }
    else if (/^\$$/.test(linea)) { moneda = 'CLP'; precio = toNum(lineas[i + 1]); }
    else {
      const mUF = linea.match(/^UF\s*([\d][\d.,]*)$/i);
      const mCLP = linea.match(/^\$\s*([\d][\d.,]*)$/);
      if (mUF) { moneda = 'UF'; precio = toNum(mUF[1]); }
      else if (mCLP) { moneda = 'CLP'; precio = toNum(mCLP[1]); }
    }
    if (!moneda || !precio) continue;
    let titulo = null;
    for (let j = i - 1; j >= 0; j--) {
      if (/^(VISTO|WhatsApp|Solicitud online de visita|\$|UF)$/i.test(lineas[j])) continue;
      titulo = lineas[j]; break;
    }
    let m2 = null, dorm = null;
    for (let k = i + 1; k < Math.min(i + 9, lineas.length); k++) {
    ...
    const precioOk = moneda === 'UF' ? (precio >= 100 && precio <= 200000) : (precio >= 5000000);
    if (precioOk && m2) {
      items.push({ link: '', titulo: (titulo || '').slice(0, 60), precio, moneda, m2_util: m2, terraza: '', dormitorios: dorm, estac: '', bodega: '', revisar: extras });
    }
  }
  return items;
}