// VERSION: v12 · 2026-08-24 · Hoja Garantías: "cobrada" es un ESTADO ("PAGADA") — se cuenta como pagada la cuota
//   (Entregada/Pendiente y "C… pagado" resuelven el importe). Antes, al ser texto, sumaba 0. Hereda v11.
...
function construirGarantias(wb, mesTexto, roster = [], mesYM = '') {
  ...
  // "cobrada" es un estado: "PAGADA" (texto) → cuenta como pagada la cuota; un número → ese importe; vacío → 0.
  const cuotasDe = f => [1, 2, 3, 4].map(i => {
    const monto = aNumero(f['cuota' + i])
    const cob = String(f['cobrada' + i] == null ? '' : f['cobrada' + i]).trim()
    const n = aNumero(cob)
    const pag = cob === '' ? 0 : (n > 0 ? n : monto)
    return { fecha: f['fecha' + i], monto, pag }
  })
