...
async function generarExcelPaola({
  ...
  construirMovimientos(wb, mesTexto, movimientos)
...
function construirMovimientos(wb, mesTexto, movimientos) {
  const ws = wb.addWorksheet('Movimientos cuenta', { views: [{ state: 'frozen', ySplit: 3 }] })
  const W = [12, 40, 14, 24]; W.forEach((w, i) => ws.getColumn(i + 1).width = w)
  pintarBanda(ws, 1, 1, 4, 'Movimientos de la cuenta · Paola Albornoz', { size: 13 })
  ws.getRow(1).height = 26
  pintarBanda(ws, 2, 1, 4, `Cartola de ${mesTexto} que envía Paola (cruzada con la liquidación).`, { fill: CLARO, color: GRIS, size: 9, bold: false })
  const cab = ['Fecha', 'Detalle', 'Monto ($)', 'Glosa / dpto']
  cab.forEach((t, i) => {
  ...
  if (!lista.length) {
    ...
    lista.forEach((m, idx) => {
      const vals = [aFecha(m.fecha) || (m.fecha ? String(m.fecha) : null), m.detalle || m.nota_cartola || '', aNumero(m.monto), m.idadmon || m.glosa || '']
      vals.forEach((v, i) => {
