...
// Hoja "Garantías": TODAS las garantías de los contratos S/SQ (fuente: datos_arriendos), ordenadas por
// inmueble, con un resumen del mes arriba. Reproduce el control tradicional: garantía pedida, entregada
// (Σ cobradas), pendiente, quién la tiene, deuda, cuotas 1-4 (fecha/monto/cobrado) y contacto.
function construirGarantias(wb, mesTexto, roster = [], mesYM = '') {
  ...
  const GC = [
    ...
    { t: 'Est',            w: 6,  al: 'center' },
    { t: 'Inmueble',       w: 30, al: 'left', wrap: true },
    { t: 'Arrendatario',   w: 24, al: 'left', wrap: true },
    { t: 'Garantía pedida',w: 13, al: 'right', money: true },
    ...
    { t: 'C1 fecha',       w: 11, al: 'center', date: true },
    { t: 'C1 monto',       w: 11, al: 'right', money: true },
    { t: 'C1 cobr.',       w: 11, al: 'right', money: true },
    { t: 'C2 fecha',       w: 11, al: 'center', date: true },
    { t: 'C2 monto',       w: 11, al: 'right', money: true },
    { t: 'C2 cobr.',       w: 11, al: 'right', money: true },
    { t: 'C3 fecha',       w: 11, al: 'center', date: true },
    { t: 'C3 monto',       w: 11, al: 'right', money: true },
    { t: 'C3 cobr.',       w: 11, al: 'right', money: true },
    { t: 'C4 fecha',       w: 11, al: 'center', date: true },
    { t: 'C4 monto',       w: 11, al: 'right', money: true },
    { t: 'C4 cobr.',       w: 11, al: 'right', money: true },
    { t: 'Ingreso',        w: 11, al: 'center', date: true },
    { t: 'Término',        w: 11, al: 'center', date: true },
    { t: 'Mail',           w: 26, al: 'left' },
    { t: 'Móvil',          w: 15, al: 'left' },
  ]
  GC.forEach((c, i) => ws.getColumn(i + 1).width = c.w)
  const NC = GC.length

  const enMes = v => {
    const d = aFecha(v); if (!d) return false
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}` === mesYM
  }

  // Orden por inmueble (natural)
  const filas = [...roster].sort((a, b) =>
    String(a.inmueble || '').localeCompare(String(b.inmueble || ''), 'es', { numeric: true, sensitivity: 'base' }))

  // Precálculo de totales y movimientos del mes
  let totPedida = 0, totEntregada = 0, totPendiente = 0, movN = 0, movVencido = 0, movCobrado = 0
  const cuotasDe = f => [1, 2, 3, 4].map(i => ({
    fecha: f['fecha' + i], monto: aNumero(f['cuota' + i]), cobr: aNumero(f['cobrada' + i]),
  }))
  for (const f of filas) {
    const pedida = aNumero(f.garantia_pedida) || 0
    const cs = cuotasDe(f)
    const entregada = cs.reduce((s, c) => s + (c.cobr || 0), 0)
    totPedida += pedida; totEntregada += entregada; totPendiente += Math.max(pedida - entregada, 0)
    for (const c of cs) if (enMes(c.fecha) && (c.monto || c.cobr)) { movN++; movVencido += (c.monto || 0); movCobrado += (c.cobr || 0) }
  }

  // Banda de título
  pintarBanda(ws, 1, 1, NC, `Garantías · contratos S/SQ · ${mesTexto}`, { size: 13 })
  ws.getRow(1).height = 26

  // Resumen del mes (fila 2, merged)
  ws.mergeCells(2, 1, 2, NC)
  const res = ws.getCell('A2')
  const f0 = n => '$' + Math.round(n).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')
  res.value = `Cuotas de garantía con vencimiento en ${mesTexto}: ${movN} — vencido ${f0(movVencido)}, cobrado ${f0(movCobrado)}.   ·   ` +
    `Total pedido (S/SQ): ${f0(totPedida)}   ·   Entregado: ${f0(totEntregada)}   ·   Pendiente: ${f0(totPendiente)}`
  ...
  for (const f of filas) {
    ...
    const vals = [
      f.idadmon, f.estado || '', f.inmueble || '', f.arrendatario || '',
      pedida || null, entregada || null, pendiente || null,
      f.quien_tiene_garantia || '', aNumero(f.deuda_garantia) || null,
    ]
    cs.forEach(c => { vals.push(aFecha(c.fecha), c.monto || null, c.cobr || null) })
    vals.push(aFecha(f.fecha_inicio), aFecha(f.termino_actual), f.mail_arrendatario || '', f.movil || '')

    vals.forEach((v, i) => {
      const c = GC[i]
      const cell = ws.getCell(r, i + 1)
      cell.value = v
      if (c.money && typeof v === 'number') cell.numFmt = FMT_NUM
      ...
      cell.border = BORDER
      // Resaltar la cuota cuyo pago cae en el mes en curso
      const esCuotaFecha = c.date && [10, 13, 16, 19].includes(i) && enMes(v)
      if (esCuotaFecha) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: CLARO } }
      else if ((r - ini) % 2) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: ZEBRA } }
    })
    r++
  }
  ...
  if (filas.length === 0) {
    ...
    const c = ws.getCell(r, 1); c.value = 'No hay contratos S/SQ con datos de garantía.'
    c.font = { name: FUENTE, size: 10, italic: true, color: { argb: GRIS } }
    return
  }

  // Totales (pedida col5, entregada col6, pendiente col7)
  const ult = r - 1
  const ft = r
  const tcell = ws.getCell(ft, 1); tcell.value = 'TOTALES'
  tcell.font = { name: FUENTE, size: 10, bold: true, color: { argb: BLANCO } }
  for (let i = 1; i <= NC; i++) {
