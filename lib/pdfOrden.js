// =====================================================================
// Generador del PDF de la Orden de Visita (pdf-lib) - v3
// - v3: SANEA todo el texto de entrada para la fuente Helvetica (Latin-1).
//   Evita el fallo real de la orden N°43: un carácter INVISIBLE de dirección
//   bidireccional (U+202A, colado al copiar el teléfono desde el móvil/WhatsApp)
//   que Helvetica no puede dibujar y hacía reventar la generación del PDF.
//   Se quitan marcas invisibles (bidi/BOM/zero-width), se pasan comillas y
//   guiones tipográficos a ASCII y se descarta cualquier carácter fuera de Latin-1.
...
// Saneador para la fuente Latin-1 (Helvetica): sin él, un carácter fuera de Latin-1
// (p. ej. la marca invisible U+202A del teléfono) lanza excepción al dibujar y aborta el PDF.
function wa(t) {
  return String(t ?? '')
    .replace(/[‘’‚′]/g, "'")
    .replace(/[“”„″]/g, '"')
    .replace(/[–—−]/g, '-')
    .replace(/…/g, '...')
    .replace(/[\u202A-\u202E\u2066-\u2069\u200B-\u200F\uFEFF]/g, '') // bidi / zero-width / BOM (invisibles)
    .replace(/[^\x00-\xFF]/g, '')                                    // cualquier otro fuera de Latin-1
}
// Limpia solo los campos de TEXTO de los datos, preservando firma (imagen), folio (número) y demás.
function limpiarDatos(data) {
  const c = { ...data }
  c.comercial = wa(data.comercial)
  c.fechaEmision = wa(data.fechaEmision)
  c.fechaVisita = wa(data.fechaVisita)
  c.horaVisita = wa(data.horaVisita)
  c.cliente = { ...(data.cliente || {}) }
  for (const k of ['nombre', 'telefono', 'email', 'rut']) c.cliente[k] = wa(c.cliente[k])
  c.propiedades = (Array.isArray(data.propiedades) ? data.propiedades : []).map(p => {
    const q = { ...p }
    for (const k of Object.keys(q)) if (typeof q[k] === 'string') q[k] = wa(q[k])
    return q
  })
  if (data.firma && typeof data.firma === 'object') {
    c.firma = { ...data.firma }
    if (typeof c.firma.nombre === 'string') c.firma.nombre = wa(c.firma.nombre)
    if (typeof c.firma.rut === 'string') c.firma.rut = wa(c.firma.rut)
    if (typeof c.firma.docLabel === 'string') c.firma.docLabel = wa(c.firma.docLabel)
  }
  return c
}
...
export async function generarOrdenPDF(data = {}) {
  data = limpiarDatos(data)   // ← v3: quita caracteres que Helvetica no puede dibujar (evita que la orden se caiga)
