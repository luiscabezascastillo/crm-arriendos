...
export async function generarOrdenPDF(data = {}) {
  ...
  page.drawText(`N° ${data.folio ?? '—'}`, { x: W - M - 90, y: y - 15, size: 12, font: bold, color: gris })
  y -= 23
  page.drawText(CORREDOR.oficina, { x: M, y: y - 8.5, size: 8.5, font, color: gris })
  if (data.fechaEmision) page.drawText(`Emitida: ${data.fechaEmision}`, { x: W - M - 130, y: y - 8.5, size: 8.5, font, color: gris })
  y -= 16
  hr()
  ...
  // Propiedades
  seccion('Propiedades autorizadas')
  const props = data.propiedades || []
  if (!props.length) {
    texto('(sin propiedades indicadas)', { size: 9, color: gris, justify: false })
  } else {
    props.forEach((p, i) => {
      const head = [p.operacion, p.tipo].filter(Boolean).join(' · ')
      const valor = p.valor ? `${p.moneda || ''} ${Number(p.valor).toLocaleString('es-CL')}`.trim() : ''
      const cols = [head, p.direccion, p.comuna, valor, p.codigo ? 'cód ' + p.codigo : ''].filter(Boolean).join('   ·   ')
      texto(`${i + 1}.  ${cols}`, { size: 9, gap: 2.5, justify: false })
    })
  }
  y -= 6
  ...
  for (const c of CONDICIONES) {
    ...
    texto(`${c.n}. ${c.titulo}.`, { f: bold, size: 9, gap: 1.5, justify: false })
