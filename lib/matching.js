...
// Point-in-polygon (ray casting). poligono: array de [lat, lng]. Punto: (lat, lng).
...
function matchPropiedad(req, pub, edificio, valorUF) {
  ...
  // 4) Zona: si el requerimiento tiene poligono, MANDA la zona (coordenadas). Si no, comuna.
  const poligono = Array.isArray(req.zona_poligono) && req.zona_poligono.length >= 3 ? req.zona_poligono : null
  if (poligono) {
    const _lat = num(pub.latitud), _lng = num(pub.longitud)
    if (_lat == null || _lng == null) return { match: false, falla: 'sin coordenadas' }
    if (!puntoEnPoligono(_lat, _lng, poligono)) return { match: false, falla: 'fuera de la zona' }
    motivos.push('dentro de la zona')
  } else if (req.comunas && req.comunas.length) {
...
function buscarRequerimientosParaPropiedad(pub, edificio, requerimientos, valorUF) {
  ...
  for (const req of (requerimientos || [])) {
    ...
    if (norm(req.etapa) === 'descartado') continue
