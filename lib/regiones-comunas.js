...
// Fuente: division politico-administrativa oficial. Orden de regiones: norte a sur.
// Uso: import { REGIONES, comunasDeRegion, regionDeComuna } from '@/lib/regiones-comunas'
...
// Lista de nombres de regiones en orden norte-sur
export const REGIONES = Object.keys(REGIONES_COMUNAS)

// Todas las comunas en un solo array (ordenadas alfabeticamente)
export const TODAS_COMUNAS = Array.from(
  new Set(Object.values(REGIONES_COMUNAS).flat())
).sort((a, b) => a.localeCompare(b, 'es'))

// Comunas de una region (array vacio si la region no existe)
export function comunasDeRegion(region) {
  return REGIONES_COMUNAS[region] || []
}

// Dada una comuna, devuelve su region (o '' si no se encuentra)
export function regionDeComuna(comuna) {
  if (!comuna) return ''
  const objetivo = String(comuna).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim()
  for (const [region, comunas] of Object.entries(REGIONES_COMUNAS)) {
    for (const c of comunas) {
      const norm = c.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim()
      if (norm === objetivo) return region
    }
  }
  return ''
}
