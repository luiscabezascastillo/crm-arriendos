// VERSION: v6 · 2026-08-26 · FIX desplegable más alto que la pantalla (p.ej. Fecha inicio en LOG/CC1, con árbol de
//   muchos años): el botón "Aceptar" quedaba por debajo del borde y no se podía pulsar. Ahora el popover se capa a
//   la altura de la ventana (maxHeight = 100vh − posición del botón) y es scrollable, y el PIE (Aceptar/Cancelar)
//   queda FIJO abajo (position:sticky), así siempre se ve. Aplica a los dos modos (absoluto y flotante). Hereda v5.
// VERSION: v5 · 2026-08-18 · Nueva marca por columna `ordenDesc`: invierte el orden del listado del desplegable
//   (para Folio de Descuentos, que el más reciente salga arriba). Solo actúa si la columna la declara; el resto
//   (CC1/LOG) sigue en ascendente. Hereda v4.
...
export function HeaderFilter({ col, movs, state, setState, open, setOpen, orden, setOrden, limpiarTodo, hayAlguno, flotante }) {
  ...
  const valores = useMemo(() => {
    ...
    // Opción por columna: `ordenDesc` invierte el listado del desplegable (p.ej. Folio de Descuentos:
    // el más alto = más reciente arriba). Sin la marca, orden ascendente como siempre (no afecta a CC1/LOG).
    if (col.ordenDesc) arr.reverse()
    return arr
  }, [movs, col])
      ...
      {abierto && (
        <>
          ...
          <div onClick={e => e.stopPropagation()} style={flotante
            ...
            : { position: 'absolute', top: 18, left: 0, zIndex: 31, background: '#fff', color: '#2C2C2A', border: '0.5px solid #D3D1C7', borderRadius: 8, boxShadow: '0 8px 26px rgba(0,0,0,0.16)', width: 272, textAlign: 'left', fontWeight: 400, maxHeight: `calc(100vh - ${fpos.top + 12}px)`, overflowY: 'auto', overflowX: 'hidden' }}>
            ...
            <div style={{ display: 'flex', gap: 6, padding: 8, borderTop: '0.5px solid #ECEAE3', background: '#FAFAF7', position: 'sticky', bottom: 0, zIndex: 2 }}>
