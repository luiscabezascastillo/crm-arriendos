...
//   pero marcada como "fuera de esta liquidación". Se retira el split "A transferir ahora / En espera" del total y del cierre
//   (v7/v10); en su lugar una NOTA SIN importe con el nº de arriendos pendientes. Los totales se calculan sobre activos en
//   cargar() (que ahora lee retenidos). Hereda v12.
// VERSION: v12 · 2026-08-10 · El texto de un depto en captación (P) se edita EN LÍNEA sobre la propia celda Arrendatario
//   (clic → escribir "RESERVADO" o lo que sea). Lo hacen Alberto/Luis/Karina al revisar. Se guarda por idadmon en
//   `captacion_etiqueta` y sale igual en el PDF; vacío = vuelve a "EN CAPTACION ARRENDATARIO". Hereda v11.
// VERSION: v11 · 2026-08-10 · Etiqueta libre de captación: si un depto en estado P tiene etiqueta en `captacion_etiqueta`
//   (lee la carta muestra ESE texto en vez de "EN CAPTACION ARRENDATARIO"). Hereda v10.
...
function recomputarBloque(b, idadmon, ovr) {
  ...
  const T = inmuebles.filter(x => !x.enEspera).reduce((a, x) => ({
...
export default function CartasPage() {
  ...
  // Editar el texto que sale en la carta para un depto en captación (P): "RESERVADO", etc.
  // Se guarda por idadmon en captacion_etiqueta; vacío = vuelve a "EN CAPTACION ARRENDATARIO".
  ...
  async function cargar(m) {
    ...
    try {
      ...
      const [rArr, rServ, rDesc, rCom, rCargos, rObs, rEnvios, rProps, rOvr, rCap] = await Promise.all([
      ...
      // Líneas EN ESPERA (retenidas por morosidad) del mes: se EXCLUYEN de los totales de esta liquidación
      // (se liquidarán en una complementaria al cobrar). La fila sigue visible para gestionarla.
      ...
      setRetenidos(Object.fromEntries([...retSet].map(id => [id, true])))
      ...
      for (const r of rows) {
        ...
        grupos[r.idprop].inmuebles.push({
          ...
          // En espera = arrendatario moroso retenido: fila visible pero FUERA de los totales de esta liquidación.
      ...
      const lista = Object.values(grupos).map(g => {
        ...
        // Los EN ESPERA se excluyen de los totales (se liquidan aparte al cobrar).
        const activos = g.inmuebles.filter(x => !x.enEspera)
        const enEsperaCount = g.inmuebles.length - activos.length
        const T = activos.reduce((a, x) => ({
          aCobrar: a.aCobrar + x.aCobrar, recibido: a.recibido + x.recibido, admon: a.admon + x.admon,
          iva: a.iva + x.iva, descuentos: a.descuentos + x.descuentos, aTransferir: a.aTransferir + x.aTransferir,
        }), { aCobrar: 0, recibido: 0, admon: 0, iva: 0, descuentos: 0, aTransferir: 0 })
        ...
        return { ...g, totales: T, transferido, diff, estado, enEsperaCount }
    ...
    <>
      ...
      <div style={{ maxWidth: 1900, margin: '0 auto', padding: 20, fontFamily: '"DM Sans", sans-serif', fontVariantNumeric: 'tabular-nums', fontFeatureSettings: '"tnum" 1' }}>
        ...
        {!cargando && visibles.map(b => {
          ...
          // Los EN ESPERA ya están FUERA de b.totales (se liquidan aparte al cobrar). Aquí, para info interna
          // de gestión, se calcula cuántos son y cuánto suman los netos retenidos (no entra en "A transferir").
          const nEspera = n0(b.enEsperaCount)
          const retTot = (b.inmuebles || []).reduce((a, x) => a + ((!x.esP && (x.enEspera || retenidos[x.idadmon])) ? n0(x.aTransferir) : 0), 0)
          return (
            <div key={b.idprop} id={'liq-' + b.idprop} style={{ scrollMarginTop: 110, border: tieneOvr ? '1.5px solid #EF4444' : '1px solid #C7D2FE', borderRadius: 10, marginBottom: 16, overflow: 'hidden', background: '#fff' }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <div style={{ minWidth: 1632 }}>
                  ...
                  {b.inmuebles.map((x, i) => {
                    ...
                    const enEsp = !x.esP && (x.enEspera || retenidos[x.idadmon])   // pendiente de cobro: visible, fuera de totales
                    ...
                    <div key={x.idadmon + i} style={{ display: 'grid', gridTemplateColumns: COLS, gap: 6, padding: '6px 12px', borderTop: '1px solid #F0EEE8', alignItems: 'center', ...(enEsp ? { background: '#FFF7ED' } : {}) }}>
                      ...
                      <div onClick={(e) => { if (puedeOverride && !x.esProp && !x.esP) { e.stopPropagation(); abrirOverride(x) } }}
                        ...
                        style={{ ...td, ...rt, ...bgP, fontWeight: 600, cursor: (puedeOverride && !x.esProp && !x.esP) ? 'pointer' : 'default', ...(x.override ? { background: '#FEF3C7', borderRadius: 4 } : {}), ...((!x.esP && (x.enEspera || retenidos[x.idadmon])) ? { color: '#94A3B8', textDecoration: 'line-through' } : {}) }}>
                  ...
                  {nEspera > 0 && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '5px 12px 5px 40px', background: '#FFF7ED', borderTop: '1px solid #FED7AA' }}>
                      <span style={{ color: '#9CA3AF', fontSize: 12 }}>↳</span>
                      <span style={{ fontSize: 12, color: '#9A3412' }}>⏸ {nEspera} arriendo(s) en espera — <b>fuera de esta liquidación</b> (no suma en los totales). Se liquidará(n) aparte cuando se cobre.{retTot > 0 && <span style={{ color: '#B45309' }}> · retenido: {fmt(retTot)}</span>}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', padding: '10px 14px', borderTop: '1px solid #E5E7EB', background: '#FAFAFA' }}>
                ...
                {nEspera > 0 && (
                  <span style={{ fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 6, background: '#FFEDD5', color: '#9A3412' }}>⏸ {nEspera} en espera (fuera de esta liquidación)</span>
                )}
