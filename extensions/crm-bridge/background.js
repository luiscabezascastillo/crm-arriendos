...
function wakeUpExtension(extensionId) {
...
async function consultarAguaviaExtension(extensionId, codigoNormalizado) {
  await wakeUpExtension(extensionId)
...
export default function ServiciosAguaPage() {
  ...
  async function iniciarConsulta() {
    ...
    for (let i = 0; i < codigos.length; i++) {
      const { idadmon, idinmue, codigo_agua, codigo_agua_normalizado } = codigos[i]
