...
const fmt = (n) => (n == null ? 'â€”' : Number(n).toLocaleString('es-CL', { maximumFractionDigits: 0 }));
const fmt2 = (n) => (n == null ? 'â€”' : Number(n).toLocaleString('es-CL', { maximumFractionDigits: 2 }));
...
export default function ValoracionesPage() {
  ...
  if (status === 'loading') return <div style={{ padding: 40 }}>Cargandoâ€¦</div>;
  if (!DIRECCION.includes(email)) return <div style={{ padding: 40 }}>Acceso restringido a DirecciÃ³n.</div>;
  ...
  async function extraerUno() {
    ...
    try {
      ...
      setPegado('');
      setAvisoP(d.precio == null || d.m2 == null ? 'Agregado, revisa: faltÃ³ precio o mÂ². ComplÃ©talo a mano.' : `âœ“ Agregado (${d.m2} mÂ², ${d.precio} ${d.moneda}).`);
  ...
  async function extraerLista() {
    ...
    try {
      ...
      setAvisoP(`âœ“ ${d.total} comparables agregados${marcados ? ` Â· âš ï¸ ${marcados} traen estac/bodega/terraza en el tÃ­tulo â€” revisa esos` : ''}.`);
    } catch (e) { setAvisoP(e.message); } finally { setParseando(false); }
  }
  ...
  async function generarPDF() {
    ...
    try {
      ...
      // mapa: geocodifica la direcciÃ³n y trae el mapa (server-side, base64)
  ...
  async function calcular(guardar) {
    ...
    if (conDatos.length < 1) { setError('Ingresa al menos un testigo con mÂ² Ãºtil y precio.'); return; }
  ...
  return (
    <div style={{ maxWidth: 1120, margin: '0 auto', padding: 24, fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 22, marginBottom: 4 }}>ValoraciÃ³n de propiedades</h1>
      <p style={{ color: '#64748b', marginTop: 0, fontSize: 13 }}>
        EstimaciÃ³n referencial por comparables homologados. No reemplaza una tasaciÃ³n bancaria.
      </p>
      ...
      <div style={card}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <div style={{ flex: 2, minWidth: 200 }}><div style={lbl}>DirecciÃ³n</div><input style={inputS} value={suj.direccion} onChange={(e) => setS('direccion', e.target.value)} /></div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 10 }}>
          <div style={{ flex: 1, minWidth: 90 }}><div style={lbl}>mÂ² Ãºtil</div><input style={inputS} type="number" value={suj.m2_util} onChange={(e) => setS('m2_util', e.target.value)} /></div>
          <div style={{ flex: 1, minWidth: 90 }}><div style={lbl}>mÂ² terraza</div><input style={inputS} type="number" value={suj.terraza} onChange={(e) => setS('terraza', e.target.value)} /></div>
          ...
          <div style={{ flex: 1, minWidth: 120 }}><div style={lbl}>AvalÃºo fiscal (UF)</div><input style={inputS} type="number" value={suj.avaluo_fiscal_uf} onChange={(e) => setS('avaluo_fiscal_uf', e.target.value)} placeholder="opcional" /></div>
        </div>
        <div style={{ marginTop: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            {fotoSujeto && <span style={{ fontSize: 12, color: '#16a34a' }}>âœ“ foto cargada</span>}
          </div>
        </div>
      </div>
      ...
      <div style={{ ...card, background: '#fffbeb', border: '1px solid #fde68a' }}>
        <div style={{ fontWeight: 600, marginBottom: 8, fontSize: 13 }}>ParÃ¡metros de homologaciÃ³n</div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 140 }}><div style={lbl}>NegociaciÃ³n (%)</div><input style={inputS} type="number" value={par.negociacion} onChange={(e) => setP('negociacion', e.target.value)} /></div>
          ...
          <div style={{ flex: 1, minWidth: 140 }}><div style={lbl}>Tolerancia mÂ² Â±%</div><input style={inputS} type="number" value={par.tol_m2} onChange={(e) => setP('tol_m2', e.target.value)} /></div>
        </div>
        <div style={{ fontSize: 11, color: '#92400e', marginTop: 6 }}>La tolerancia descarta testigos cuyo tamaÃ±o se aleje del sujeto (ej. Â±40% de la sup. ponderada) â€” asÃ­ no se cuela un dÃºplex entre deptos chicos.</div>
      </div>
      ...
      <div style={card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          <button onClick={addT} style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 6, padding: '6px 12px', cursor: 'pointer', fontSize: 13 }}>+ Fila vacÃ­a</button>
        </div>

        <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, padding: 12, marginBottom: 12 }}>
          <div style={{ fontSize: 12, color: '#1e40af', marginBottom: 6 }}>Pega un aviso individual (Ctrl+A en la pÃ¡gina) o el <b>listado completo</b> del sector (con la consola: <code>copy(document.body.innerText.slice(0,8000))</code>):</div>
          <textarea value={pegado} onChange={(e) => setPegado(e.target.value)} rows={3} placeholder="Pega aquÃ­â€¦" style={{ ...inputS, fontFamily: 'monospace', fontSize: 12, resize: 'vertical' }} />
        ...
        <div style={{ display: 'grid', gap: 6 }}>
          {testigos.map((t, i) => (
            <div key={i} style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'flex-end', padding: 8, background: t.revisar ? '#fef3c7' : '#f8fafc', borderRadius: 8, border: t.revisar ? '1px solid #fcd34d' : '1px solid transparent' }}>
              <div style={{ flex: 3, minWidth: 180 }}><div style={lbl}>Link {t.revisar ? 'âš ï¸ revisar estac/bodega' : ''}</div><input style={inputS} value={t.link} onChange={(e) => setT(i, 'link', e.target.value)} placeholder="https://â€¦MLC-â€¦" /></div>
              <div style={{ flex: 1, minWidth: 70 }}><div style={lbl}>mÂ² Ãºtil</div><input style={inputS} type="number" value={t.m2_util} onChange={(e) => setT(i, 'm2_util', e.target.value)} /></div>
              ...
              <button onClick={() => delT(i)} style={{ background: '#fef2f2', border: '1px solid #fecaca', color: '#b91c1c', borderRadius: 6, padding: '7px 9px', cursor: 'pointer' }}>âœ•</button>
            </div>
          ))}
        </div>
        <button onClick={() => calcular(false)} disabled={cargando} style={{ marginTop: 14, padding: '10px 20px', background: '#0f172a', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, cursor: 'pointer', opacity: cargando ? 0.6 : 1 }}>{cargando ? 'Calculandoâ€¦' : 'Calcular'}</button>
      </div>
        ...
        <>
          {res.estimacion && (
            <div style={{ ...card, background: '#ecfdf5', border: '1px solid #a7f3d0' }}>
              <div style={{ fontSize: 12, color: '#065f46' }}>EstimaciÃ³n (mediana UF/mÂ² homologado Ã— sup. ponderada {res.estimacion.sup_ponderada_sujeto} mÂ²)</div>
              <div style={{ fontSize: 28, fontWeight: 700, color: '#065f46' }}>{fmt(res.estimacion.valor_uf)} UF{res.estimacion.valor_clp && <span style={{ fontSize: 15, fontWeight: 400, color: '#047857' }}>{'  '}â‰ˆ ${fmt(res.estimacion.valor_clp)}</span>}</div>
              <div style={{ fontSize: 13, color: '#047857' }}>Rango {fmt(res.estimacion.rango_uf[0])}â€“{fmt(res.estimacion.rango_uf[1])} UF Â· {fmt2(res.estimacion.uf_m2_mediana)} UF/mÂ²{res.vs_avaluo && <span> Â· {fmt2(res.vs_avaluo.ratio)}Ã— el avalÃºo fiscal</span>}</div>
            </div>
          )}
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
            <div style={kpi}><div style={{ fontSize: 11, color: '#64748b' }}>Testigos usados</div><div style={{ fontSize: 22, fontWeight: 700 }}>{res.totales.usados}</div><div style={{ fontSize: 11, color: '#94a3b8' }}>de {res.totales.testigos} Â· {res.totales.descartados} extremos fuera</div></div>
            <div style={kpi}><div style={{ fontSize: 11, color: '#64748b' }}>Sup. ponderada media</div><div style={{ fontSize: 22, fontWeight: 700 }}>{fmt(res.stat_sup?.media)} mÂ²</div><div style={{ fontSize: 11, color: '#94a3b8' }}>mediana {fmt(res.stat_sup?.mediana)}</div></div>
            <div style={kpi}><div style={{ fontSize: 11, color: '#64748b' }}>UF/mÂ² homologado</div><div style={{ fontSize: 22, fontWeight: 700 }}>{fmt2(res.stat_uf_m2?.media)}</div><div style={{ fontSize: 11, color: '#94a3b8' }}>mediana {fmt2(res.stat_uf_m2?.mediana)} Â· p25â€“p75 {fmt2(res.stat_uf_m2?.p25)}â€“{fmt2(res.stat_uf_m2?.p75)}</div></div>
          </div>
          ...
          {res.comparables_descartados?.length > 0 && (
            <div style={card}>
              <button onClick={() => setVerDesc(!verDesc)} style={{ background: 'none', border: 'none', color: '#2563eb', cursor: 'pointer', fontSize: 14, padding: 0 }}>{verDesc ? 'â–¼' : 'â–¶'} Ver {res.comparables_descartados.length} descartados (extremos)</button>
              {verDesc && <div style={{ marginTop: 10 }}><Tabla filas={res.comparables_descartados} /></div>}
            </div>
          )}
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            <button onClick={() => calcular(true)} disabled={guardando} style={{ padding: '10px 20px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, cursor: 'pointer', opacity: guardando ? 0.6 : 1 }}>{guardando ? 'Guardandoâ€¦' : 'Guardar valoraciÃ³n'}</button>
            {guardadoId && <span style={{ color: '#16a34a', fontSize: 13 }}>âœ“ Guardada (folio #{guardadoId})</span>}
            {guardadoId && (
              <button onClick={generarPDF} disabled={genPdf} style={{ padding: '10px 20px', background: '#b91c1c', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, cursor: 'pointer', opacity: genPdf ? 0.6 : 1 }}>
                {genPdf ? 'Generandoâ€¦' : 'ðŸ“„ Generar PDF'}
              </button>
            )}
          </div>
        </>
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ borderCollapse: 'collapse', width: '100%' }}>
        <thead><tr><th style={th}>Testigo</th><th style={th}>Sup.pond.</th><th style={th}>Oferta UF</th><th style={th}>Ajuste</th><th style={th}>Homologado</th><th style={th}>UF/mÂ²</th></tr></thead>
        <tbody>
          {filas.map((c, i) => (
            <tr key={i}>
              <td style={td}>{c.link ? <a href={c.link} target="_blank" rel="noreferrer" style={{ color: '#2563eb', textDecoration: 'none' }}>{(c.titulo || 'Ver aviso').slice(0, 32)} â†—</a> : <span style={{ color: '#94a3b8' }}>{(c.titulo || 'manual').slice(0, 32)}</span>}</td>
              <td style={td}>{fmt(c.sup_ponderada)}</td>
              <td style={td}>{fmt(c.oferta_uf)}</td>
              <td style={td}>{c.ajuste_amenidades ? (c.ajuste_amenidades > 0 ? '+' : '') + fmt(c.ajuste_amenidades) : 'â€”'}</td>
