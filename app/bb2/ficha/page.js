...
//   IVA/Total auto 19% editables). Guardar borrador (claves limpias, merge). Candado HECHO (Desbloquear: Dirección).
...
const PERS = [['prop', 'Propietario'], ['coprop', 'Co-propietario'], ['arr', 'Arrendatario 1'], ['arr2', 'Arrendatario 2'], ['aval', 'Aval']]
...
function BB2Ficha() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        let realId = qid
        if (qid === 'nuevo') {
          const jn = await (await fetch('/api/bb2/nuevo-id')).json()
          realId = jn.siguiente || 'R00001'
        }
        const j = await (await fetch('/api/bb2/recuperar?id=' + encodeURIComponent(realId))).json()
        if (!vivo) return
        if (j.error) { setMsg({ t: 'error', x: j.error }); setCargando(false); return }
        setId(realId); setModel(j.model); setHecho(!!j.hecho); setExiste(!!j.existe)
  ...
  async function guardar(desbloquear = false) {
    ...
    try {
      const res = await fetch('/api/bb2/guardar', {
      ...
      if (qid === 'nuevo' && j.id) router.replace('/bb2/ficha?id=' + j.id)
  ...
  async function desbloquear() {
    ...
    setGuardando(true); setMsg(null)
    try {
      const res = await fetch('/api/bb2/guardar', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, accion: 'desbloquear' }) })
      const j = await res.json()
      if (!res.ok || j.error) { setMsg({ t: 'error', x: j.error || ('Error ' + res.status) }); return }
  ...
  if (status === 'authenticated' && !autorizado) return <div style={{ minHeight: '100vh' }}><TopNav /><div style={{ padding: 40, color: '#b91c1c' }}>BB2 está restringido a Dirección y Anthony.</div></div>
      ...
      <div style={{ maxWidth: 1320, margin: '0 auto', padding: 18, fontFamily: 'system-ui, sans-serif' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: .5 }}>BB2 · Ficha de arriendo</span>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <input value={recuperarId} onChange={e => setRecuperarId(e.target.value)} placeholder="R00xxx"
              onKeyDown={e => { if (e.key === 'Enter' && recuperarId.trim()) router.push('/bb2/ficha?id=' + recuperarId.trim().toUpperCase()) }}
              style={{ ...inS(false), width: 110 }} />
            <button onClick={() => recuperarId.trim() && router.push('/bb2/ficha?id=' + recuperarId.trim().toUpperCase())} style={btn('#334155')}>Recuperar</button>
            <button onClick={() => router.push('/bb2/ficha?id=nuevo')} style={btn('#0C447C')}>+ Nueva</button>
            <button onClick={() => router.push('/bb2')} style={{ ...btn('#e5e7eb'), color: '#374151' }}>← Listado</button>
          </div>
        </div>
        ...
        {cargando || !model ? <div style={{ ...card, color: '#888' }}>Cargando ficha…</div> : (
          <>
            ...
            <div style={card}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                {campo('Tipo de contrato', model.tipo, v => setTop('tipo', v), { w: 220 })}
            ...
            <div style={card}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                ...
                {campo('Monto (renta)', model.inmueble.monto, v => setInm('monto', v), { w: 130 })}
                {campo('Garantía', model.inmueble.garantia, v => setInm('garantia', v), { w: 130 })}
                {campo('Comienzo', model.inmueble.inicio, v => setInm('inicio', v), { w: 120 })}
                {campo('Finalización', model.inmueble.fin, v => setInm('fin', v), { w: 120 })}
                {campo('Multas', model.inmueble.multas, v => setInm('multas', v), { w: 160 })}
                ...
                {campo('Pre-aviso', model.inmueble.pre_aviso, v => setInm('pre_aviso', v), { w: 110 })}
                {campo('Ajuste', model.inmueble.ajuste, v => setInm('ajuste', v), { w: 110 })}
                {campo('Renovación', model.inmueble.renovacion, v => setInm('renovacion', v), { w: 120 })}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 12 }}>
              {bloqueComision('comD', 'Propietario', '#0C447C')}
              {bloqueComision('comA', 'Arrendatario', '#0e7490')}
            </div>
...
export default function BB2FichaPage() {
  return (
