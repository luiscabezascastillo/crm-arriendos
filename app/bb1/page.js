'use client'
// VERSION: v1 · 2026-08-12 · app/bb1/page.js — BB1 (VENTAS · corretaje). LISTADO de solo lectura sobre el LOG
//   (id_lcc 'V%'), leído desde raw_data. Clonado de BB2 con etiquetas de venta (vendedor/comprador/precio).
//   Buscador + filtro por ejecutivo + filtro por estado (HECHO/pendiente) + recuento. Filas clicables → ficha.
//   Acceso: Dirección + Anthony (Legal) durante la construcción.
...
export default function BB1Page() {
  ...
  if (status === 'authenticated' && !autorizado) {
    return <div style={{ minHeight: '100vh' }}><TopNav /><div style={{ padding: 40, color: '#b91c1c' }}>BB1 está restringido a Dirección y Anthony durante su construcción.</div></div>
  }
    ...
    <div style={{ minHeight: '100vh', background: '#f4f6f9' }}>
      ...
      <div style={{ maxWidth: 1500, margin: '0 auto', padding: 18, fontFamily: 'system-ui, sans-serif' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap', marginBottom: 4 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: .5 }}>BB1 · Operaciones comerciales</span>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1a1a2e', margin: 0 }}>Ventas (corretaje)</h1>
          <button onClick={() => router.push('/bb1/ficha?id=nuevo')} style={{ padding: '7px 14px', borderRadius: 8, border: 'none', background: '#0C447C', color: '#fff', fontSize: 12.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', marginLeft: 'auto' }}>+ Nueva operación</button>
        </div>
        <div style={{ fontSize: 13, color: '#888', marginBottom: 16 }}>Solo corretaje (comisión de compraventa). Lectura del LOG (id V00xxx). Datos tomados de raw_data; se depurarán en la estabilización.</div>

        {/* Barra de filtros */}
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 12 }}>
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar Id, inmueble, vendedor, comprador…"
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1200 }}>
                    <thead>
                      <tr>
                        ...
                        <th style={th}>Escritura</th>
                        <th style={{ ...th, textAlign: 'right' }}>Com. vendedor</th>
                        <th style={{ ...th, textAlign: 'right' }}>Com. comprador</th>
                        <th style={{ ...th, textAlign: 'center' }}>Estado</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filtradas.map((r, i) => (
                        <tr key={r.id || i} onClick={() => r.id && router.push('/bb1/ficha?id=' + r.id)} title="Abrir ficha"
