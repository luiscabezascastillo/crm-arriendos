// VERSION: v5 Â· 2026-07-15 Â· PARTE 2 (filtros): sustituye el filtro "contiene" por filtro tipo Excel
//   (casillas de valores + buscador + "solo" / "mostrar todos") en cada cabecera, filtrando en cliente
//   sobre las 6.7k ya cargadas. En UNIQUE CONCEPT, ademÃ¡s, chips de categorÃ­a de color (Todos /
//   Identificados / Sin identificar / Cargos), combinables con las casillas. Los filtros SOLO filtran.
// VERSION: v4 Â· 2026-07-15 Â· PARTE 1 (cimiento de filtros): carga TODAS las filas de `bi` (paginado
...
// VERSION: v3 Â· 2026-07-15 Â· Segundo nivel de permiso: Anthony/Neika/Adalis/Fabiola pueden IDENTIFICAR
//   abonos (asociar RUTâ†’IDADMON) por el drawer "âž• RUT" en modo manual (sin sugerencias). El rellenado
//   del movimiento lo hace el endpoint asociar-rut (server-side), asÃ­ no dependen de escritura en `bi`.
//   EdiciÃ³n libre del resto del BI sigue siendo solo DirecciÃ³n/Karina. Resto igual que v2.
// VERSION: v2 Â· 2026-07-09 Â· gate de escritura (solo DirecciÃ³n y Karina) + columna LIQ. MES2 editable con validaciÃ³n AAMM
'use client'
...
// â”€â”€ Permisos de ESCRITURA del BI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// reservado a DirecciÃ³n y Karina, la MISMA lista que preparar-mes y EMAILS.
// El match es case-insensitive y sin espacios (evita el caso de correos con
// variantes que ya rompiÃ³ permisos antes). Nota: este gate es de INTERFAZ;
...
// â”€â”€ Segundo nivel: SOLO pueden IDENTIFICAR abonos (asociar RUTâ†’IDADMON por el drawer
// "âž• RUT", en modo manual sin sugerencias). NO editan nada mÃ¡s del BI.
// âš  EMAILS EXACTOS: verifica cada uno con el SELECT que te paso (la coincidencia debe ser
// EXACTA; ya nos mordiÃ³ antes la "m" de mÃ¡s de Anthony). Todo en minÃºsculas.
const ASOCIA_EMAILS = [
...
// AAMM de 4 dÃ­gitos, meses 01â€“12 (para validar la reasignaciÃ³n de LIQ. MES2).
...
const LIMITE = 50
// Con la carga completa, por defecto se PINTAN solo las N mÃ¡s recientes (para no volcar miles de
// inputs de golpe). "Ver todo" pinta las 6.7k. El filtrado (Parte 2) mostrarÃ¡ todas las que casen.
const TOPE_DEFECTO = 300

// Extrae el RUT (dÃ­gitos-guiÃ³n-verificador) del texto del detalle del movimiento.
...
const COLS = [
  ...
  { key: 'n_doc',                  h: 'NÂ° Doc',         ro: true, w: 86,  align: 'left',  filt: true },
...
// IDADMON vÃ¡lido: A + 5 dÃ­gitos (ej. A00819).
const esIdadmonValido = (uc) => /^A\d{5}$/.test(String(uc ?? '').trim().toUpperCase())

// LIQ. MES2 (AAMM) segÃºn la fecha de hoy (hora de Chile):
//   dÃ­a >= 23 -> mes actual + 1   Â·   dÃ­a <= 22 -> mes actual
// Ej.: 23-jun -> 2607 Â· del 24-jun al 22-jul -> 2607 Â· 23-jul -> 2608.
...
export default function BiVista() {
  ...
  // Â¿El usuario logueado puede EDITAR el BI? (DirecciÃ³n y Karina). Normalizado.
  const emailSesion = (session?.user?.email || '').trim().toLowerCase()
  const puedeEditar = EDIT_EMAILS.includes(emailSesion)
  // Puede IDENTIFICAR abonos (asociar RUT): ediciÃ³n total, o el segundo nivel (los 4).
  ...
  const [filtros, setFiltros] = useState({})        // { col: Set(valores marcados) } â€” filtro Excel
  const [catFiltro, setCatFiltro] = useState('todos') // chips: todos|identificados|sinid|cargos
  const [menuF, setMenuF] = useState(null)          // { key, x, y } â€” menÃº de filtro abierto
  const [busca, setBusca] = useState('')            // buscador dentro del menÃº
  ...
  // Valor de una celda para el filtro (vacÃ­o -> "(vacÃ­o)").
  const valorCelda = (r, key) => { const v = r[key]; return (v ?? '') === '' ? '(vacÃ­o)' : String(v) }
  ...
  // CategorÃ­a de la fila para los chips de color (misma lÃ³gica que colorFila / la leyenda).
  ...
  // Â¿La columna tiene un filtro de casillas parcial (algunas desmarcadas)?
  ...
  const fetchInitial = async (fActuales = filtros) => {
    ...
    // Carga completa: Supabase devuelve mÃ¡x. 1000 por consulta, asÃ­ que paginamos por rangos
  ...
  // â”€â”€ Autorelleno de LIQ. MES2: DESACTIVADO (Parte 1, opciÃ³n A) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Antes, al abrir la vista se escribÃ­a el mes en curso a las filas sin valor. Con la carga
  // completa eso dispararÃ­a miles de escrituras al abrir. Como esta vista pasa a ser de solo
  // lectura para el filtrado, NO se autorellena nada. (La reasignaciÃ³n manual de LIQ. MES2 por
  // DirecciÃ³n/Karina en su celda sigue funcionando igual.)
  const liqDoneRef = useRef(new Set())   // conservado por compatibilidad; ya no se usa para escribir

  // Guarda primero lo que se estÃ© editando (celda con foco) y LUEGO refresca.
  // Sin esto, si el usuario escribe en una celda y pulsa el botÃ³n sin salir
  ...
  // check1 (saltos/duplicados) sobre la secuencia COMPLETA, siempre â€” asÃ­ es correcto.
  ...
  // Aplica los filtros: primero la categorÃ­a (chips de color), luego las casillas por columna.
  ...
  // QuÃ© filas se PINTAN. check1 se calcula sobre TODA la secuencia (arriba), pero por defecto
  // solo mostramos las recientes para no volcar miles de inputs. "Ver todo" las pinta todas.
  // (En la Parte 2, con filtro activo se mostrarÃ¡n todas las que casen.)
  ...
  const guardarCelda = async (id, k, valor) => {
    if (!puedeEditar) { flash('Solo DirecciÃ³n y Karina pueden editar el BI'); return }
  ...
  // â”€â”€ Asociar RUT a IDADMON en bi_admon (origen: cuentas) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const abrirAsociar = async (r) => {
    ...
    if (!rut) { flash('No se encontrÃ³ un RUT en el detalle'); return }
    ...
    } catch { setAsocErr('Error de conexiÃ³n') }
    setAsocLoading(false)
  }

  const asociarRut = async (idadmon) => {
    const id = String(idadmon || '').trim().toUpperCase()
    if (!/^A\d{5}$/.test(id)) { setAsocErr('IDADMON no vÃ¡lido (debe ser Axxxxx, ej. A00819)'); return }
    ...
    try {
      ...
      // asÃ­ funciona tambiÃ©n para quien no tiene escritura directa en `bi`.
      ...
      if (d.rellenado === false && d.errorRelleno) {
        setAsocErr('Se asociÃ³ el RUT pero no se pudo rellenar el movimiento: ' + d.errorRelleno)
        setAsocGuardando(false); return
      }
      // Reflejar en la vista el IDADMON que el endpoint ya escribiÃ³ en unique_concept.
      setRows(rs => rs.map(r => r.id === row.id ? { ...r, unique_concept: id } : r))
      flash(d.yaExistia ? `Ya estaba asociado (${rut} â†’ ${id})` : `âœ“ Asociado ${rut} â†’ ${id}`)
      setAsocGuardando(false); setAsocOpen(null)
    } catch { setAsocErr('Error de conexiÃ³n'); setAsocGuardando(false) }
  }

  const copiarFaltan = async () => {
    if (!puedeEditar) { flash('Solo DirecciÃ³n y Karina pueden editar el BI'); return }
    if (copiando) return
    if (!confirm('Â¿Copiar a CUENTAS todos los movimientos en FALTA con IDADMON vÃ¡lido?')) return
    ...
    try {
      ...
      if (d.invalidos?.length) {
        ...
        setError(
          `ERROR: se ha colocado "FALTA" a ${d.invalidos.length} movimiento(s) NO asociado(s) a un IDADMON vÃ¡lido (Axxxxx). ` +
          `NO se han pasado a CARTOLAS y siguen en FALTA. CorrÃ­gelos en BI` + (regs ? ` (Reg: ${regs}).` : '.')
        )
      }
      flash(`âœ“ ${d.copiados} copiado(s) a CUENTAS`)
      fetchInitial()
    } catch (err) {
  ...
  const usarEnUniqueConcept = async (d) => {
    ...
    // si ya hay un texto de contabilidad (empieza por "num Axxxxx"), pedir confirmaciÃ³n
  ...
  const copiarTexto = async (t) => {
    ...
    try {
      ...
      flash('âœ“ Texto copiado')
    } catch { /* si falla, no rompemos nada */ }
  }

  if (status === 'loading' || loading)
    return (<><TopNav /><div style={{ padding: 60, textAlign: 'center', color: '#888', fontSize: 14 }}>Cargando movimientosâ€¦</div></>)
  ...
  const cell = (r, c) => {
    if (c.key === '_check1') return r._check1 == null
      ? <span style={{ color: '#B4B2A9' }}>â€”</span>
    ...
    if (c.key === '_descuentos') {
      ...
      if (!tieneId && !esEgreso) return <span style={{ color: '#B4B2A9' }}>â€”</span>
      if (!puedeEditar) return <span style={{ color: '#B4B2A9' }}>â€”</span>
      ...
      return (
        <button onClick={(e) => abrirDescuentos(r, e)}
          ...
          style={{ border: '0.5px solid #C8C5BC', background: abierto ? '#E6F1FB' : '#fff', borderRadius: 6, cursor: 'pointer', fontSize: 11, padding: '2px 7px' }}>ðŸ“‹</button>
      )
    }
    if (c.key === '_asociar') {
      const esAbono = num(r.abonos) > 0
      const rut = extraerRut(r.detalle_movimiento)
      if (!esAbono || !rut) return <span style={{ color: '#B4B2A9' }}>â€”</span>
      if (!puedeAsociar) return <span style={{ color: '#B4B2A9' }}>â€”</span>
      ...
      return (
        <button onClick={() => abrirAsociar(r)}
          title={`Asociar el RUT ${rut} a un IDADMON en bi_admon (busca en CUENTAS a quÃ© contrato pagÃ³ antes)`}
          style={{ border: '0.5px solid ' + (resuelto ? '#C8C5BC' : '#9BD7C2'), background: abierto ? '#E1F5EE' : (resuelto ? '#fff' : '#F0FAF6'), color: resuelto ? '#8A8780' : '#085041', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontWeight: 700, padding: '2px 7px' }}>âž• RUT</button>
      )
    }
    // LIQ. MES2: mes de liquidaciÃ³n al que se imputa el pago. Editable SOLO por
    // DirecciÃ³n/Karina (reasignar excepciones del corte del dÃ­a 23). Valida AAMM.
    // Si la celda estÃ¡ vacÃ­a, muestra en gris el valor que le tocarÃ­a por la regla
    // del dÃ­a (liqMes2Actual), sin escribirlo aquÃ­ (de eso se ocupa el autollenado).
      ...
      return (
        <input value={actual} title={vacio ? `Sin asignar (por regla: ${liqMes2Actual()})` : actual}
          ...
          onBlur={e => {
            ...
            if (val !== '' && !esAAMM(val)) {                          // formato invÃ¡lido -> revertir
              onLocal(r.id, c.key, orig)
              flash('LIQ. MES2 debe ser AAMM (p. ej. 2607)')
              return
            }
            guardarCelda(r.id, c.key, val)                            // vÃ¡lido o vaciado
          }}
    ...
    if (!c.ro) {
      ...
      return (
        <input value={r[c.key] ?? ''} title={amarillo ? 'Falta teclear el IDADMON (A+5 dÃ­gitos)' : (r[c.key] ?? '')}
          placeholder={amarillo ? 'IDADMONâ€¦' : ''}
    ...
    if (c.money) { const s = fmt(r[c.key]); return <span title={s || ''} style={{ color: s && c.color ? c.color : '#2C2C2A' }}>{s || 'â€”'}</span> }
    return <span title={r[c.key] ?? ''}>{r[c.key] ?? 'â€”'}</span>
  }
  ...
  const toggleValor = (key, valor) => setFiltros(prev => {
    // Si no habÃ­a filtro en esta columna, partimos de "todos marcados" y quitamos el que se toca.
    ...
    return (
      <>
        ...
        <div style={{ position: 'fixed', left, top: menuF.y, width: W, background: '#fff', border: '0.5px solid #B4B2A9', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,.15)', zIndex: 41, fontSize: 12, padding: 8 }}>
          ...
          <input autoFocus placeholder="Buscarâ€¦" value={busca} onChange={e => setBusca(e.target.value)}
  ...
  const renderPopDescuentos = () => {
    ...
    const unico = !descLoading && descRows.length === 1   // precarga: Ãºnico candidato
    return (
      <>
        ...
        <div style={{ position: 'fixed', left, top: descOpen.y, width: W, maxHeight: 420, overflow: 'auto', background: '#fff', border: '0.5px solid #B4B2A9', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,.15)', zIndex: 41, fontSize: 12 }}>
          <div style={{ padding: '8px 10px', borderBottom: '0.5px solid #EDEBE4', fontWeight: 600, color: '#5F5E5A', position: 'sticky', top: 0, background: '#fff' }}>
            {esImporte
              ? <>Egreso de <b>{fmt(cargo)}</b> Â· descuentos con transferir = {fmt(cargo)}</>
              : <>Descuentos de {String(r?.idadmon2 || '').trim().toUpperCase()}</>}
          </div>

          {/* buscador para navegar (por IDADMON, NÂ° o texto) */}
          <div style={{ padding: '8px 10px', borderBottom: '0.5px solid #F0EEE8', display: 'flex', gap: 6 }}>
            <input value={descQuery} onChange={e => setDescQuery(e.target.value)}
              ...
              placeholder="Buscar por IDADMON, NÂ° o textoâ€¦"
            ...
            {esImporte && (
              <button onClick={() => { setDescQuery(''); buscarDescuentos({ monto: Math.round(cargo) }) }}
                ...
                style={{ border: '0.5px solid #C8C5BC', background: '#fff', color: '#5F5E5A', borderRadius: 6, cursor: 'pointer', fontSize: 11, padding: '4px 8px' }}>â†º importe</button>
            )}
          </div>

          {descLoading
            ? <div style={{ padding: 12, color: '#888780' }}>Cargandoâ€¦</div>
            : descRows.length === 0
              ? <div style={{ padding: 12, color: '#888780' }}>
                  {esImporte ? 'NingÃºn descuento con ese importe. Usa el buscador para localizarlo.' : 'Sin resultados.'}
                </div>
              : descRows.map((d, i) => (
                <div key={i} style={{ padding: '8px 10px', borderBottom: '0.5px solid #F0EEE8', display: 'flex', gap: 8, alignItems: 'flex-start', background: unico ? '#F3FAF6' : '#fff' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 11, color: '#888780', marginBottom: 2 }}>
                      NÂ° {d.num || 'â€”'} Â· {d.idadmon || ''} Â· {d.tipo || ''} Â· transferir {fmt(d.monto_a_transferir)}{unico ? ' Â· (Ãºnico candidato)' : ''}
                    </div>
                  ...
                  {d.texto_para_contabilidad && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
                      ...
                      <button onClick={() => copiarTexto(d.texto_para_contabilidad)} title="Solo copiar al portapapeles"
                        style={{ border: 'none', background: '#E6F1FB', color: '#0C447C', borderRadius: 6, cursor: 'pointer', fontSize: 11, padding: '4px 8px' }}>ðŸ“‹</button>
                    </div>
                  )}
                </div>
              ))
    ...
    <>
      ...
      <div style={{ maxWidth: 1640, margin: '0 auto', padding: '18px 20px 30px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, gap: 10, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 600, margin: '0 0 2px', color: '#2C2C2A' }}>BI Â· Movimientos (tabla bi)</h1>
            <div style={{ fontSize: 12, color: '#888780' }}>recientes abajo Â· carga completa{hayFiltroActivo ? ' Â· filtrado (check1 oculto)' : ''}{puedeEditar ? ' Â· edita desde UNIQUE CONCEPT Â· los cambios se guardan solos al salir de la celda (âœ“ Guardado)' : ' Â· modo solo lectura'}</div>
          </div>
          <button onClick={() => router.push('/procesos/bi')}
            style={{ fontSize: 12, padding: '6px 14px', borderRadius: 8, border: '0.5px solid #D3D1C7', background: '#fff', cursor: 'pointer', color: '#2C2C2A', whiteSpace: 'nowrap' }}>â† Cargar cartola</button>
        </div>
        ...
        {!puedeEditar && (
          <div style={{ marginBottom: 10, padding: '8px 12px', borderRadius: 8, background: '#FBF7EC', border: '0.5px solid #E6D58A', color: '#8a6d1e', fontSize: 12 }}>
            Modo solo lectura â€” la ediciÃ³n del BI (asociar RUT, IDADMON, mes de liquidaciÃ³n, copiar a CUENTAS) estÃ¡ reservada a DirecciÃ³n y Karina.
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, marginBottom: 10, fontSize: 11, color: '#5F5E5A', alignItems: 'center' }}>
          ...
          {!hayFiltroActivo && errChk > 0 && <span style={{ color: '#9B1C1C', fontWeight: 600 }}>âš  check1 â‰  0 en {errChk}</span>}
          {savingId && <span style={{ color: '#1D9E75' }}>guardandoâ€¦</span>}
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12, alignItems: 'center' }}>
          <button onClick={guardarYRefrescar} disabled={refreshing}
            ...
            {refreshing ? 'Actualizandoâ€¦' : 'ðŸ”„ Refrescar lista'}
          </button>
          <span style={{ width: 1, height: 22, background: '#D3D1C7', margin: '0 4px' }} />
          {[
            ['Verificar si en CUENTAS', 'Verifica quÃ© ingresos ya estÃ¡n en CUENTAS', null],
            ['Copiar FALTAN a CUENTAS', 'Exporta a CUENTAS los marcados FALTA (solo IDADMON vÃ¡lido)', copiarFaltan],
          ...
          <button onClick={() => setVerTodos(v => !v)}
            title={verTodos ? 'Mostrar solo las mÃ¡s recientes' : 'Mostrar las 6.7k filas (puede ir mÃ¡s lento)'}
        ...
        <div ref={scrollRef} onScroll={onScroll} style={{ overflow: 'auto', maxHeight: '72vh', border: '0.5px solid #D3D1C7', borderRadius: 8 }}>
          <table style={{ borderCollapse: 'separate', borderSpacing: 0, fontSize: 11, minWidth: 1600 }}>
                    ...
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                      ...
                      {c.filt && (
                        <button onClick={(e) => abrirFiltro(c, e)} title="Filtrar"
                          style={{ border: 'none', background: (colFiltrada(c.key) || (c.key === 'unique_concept' && catFiltro !== 'todos')) ? '#1D9E75' : 'transparent', color: (colFiltrada(c.key) || (c.key === 'unique_concept' && catFiltro !== 'todos')) ? '#fff' : '#888780', borderRadius: 4, cursor: 'pointer', fontSize: 10, lineHeight: 1, padding: '2px 4px' }}>â–¾</button>
                      )}
                    </span>
                  </th>
                ))}
            ...
            <tbody>
              {!verTodos && filas.length > TOPE_DEFECTO && <tr><td colSpan={COLS.length} style={{ padding: 6, textAlign: 'center', color: '#B4B2A9', fontSize: 10 }}>â€” mostrando las {TOPE_DEFECTO} mÃ¡s recientes de {filas.length} Â· Â«Ver todoÂ» arriba â€”</td></tr>}
              {(verTodos || filas.length <= TOPE_DEFECTO) && filas.length > 0 && <tr><td colSpan={COLS.length} style={{ padding: 6, textAlign: 'center', color: '#B4B2A9', fontSize: 10 }}>â€” inicio de la tabla â€”</td></tr>}
              {visibles.map((r) => (
                <tr key={r.id}>
                  {COLS.map((c, ci) => (
        ...
        <div style={{ fontSize: 11, color: '#888780', marginTop: 8 }}>
          {visibles.length} de {filas.length} fila(s){hayFiltroActivo ? ' (filtradas)' : ''} Â· carga completa Â· check1 0 (verde) ok; rojo = posible lÃ­nea saltada/duplicada (solo sin filtros).
        </div>
      </div>
      {renderFiltro()}
      {renderPopDescuentos()}
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 'min(520px, 94vw)', maxHeight: '86vh', display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: 12, boxShadow: '0 12px 40px rgba(0,0,0,0.25)', zIndex: 71 }}>
            <div style={{ padding: '14px 18px', borderBottom: '0.5px solid #E4E2DA', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: '#5F5E5A' }}>RUT <b>{asocOpen.rut}</b> â€” se guardarÃ¡ en <code>bi_admon</code> para autocompletar sus abonos futuros.</div>
              </div>
            ...
            <div style={{ padding: '14px 18px', overflow: 'auto' }}>
              ...
              {asocOpen.soloManual ? (
                <div style={{ padding: '10px 12px', borderRadius: 8, background: '#F1EFE8', color: '#5F5E5A', fontSize: 12, marginBottom: 4 }}>
                  ...
                  quedarÃ¡ asociado y sus abonos futuros se reconocerÃ¡n solos.
                </div>
              ) : (
                <>
                  <div style={{ fontSize: 12, color: '#5F5E5A', fontWeight: 600, marginBottom: 6 }}>SegÃºn pagos anteriores en CUENTAS:</div>
                  {asocLoading && <div style={{ padding: 16, textAlign: 'center', color: '#888780', fontSize: 13 }}>Buscando en el historialâ€¦</div>}
                  {!asocLoading && asocCands.length === 0 && (
                    <div style={{ padding: '10px 12px', borderRadius: 8, background: '#FBF7EC', color: '#8a6d1e', fontSize: 12, marginBottom: 12 }}>
                      Este RUT no aparece en CUENTAS con ningÃºn IDADMON. EscrÃ­belo a mano abajo.
                    </div>
                  )}
                  {!asocLoading && asocCands.map((c) => (
                    <button key={c.idadmon} onClick={() => asociarRut(c.idadmon)} disabled={asocGuardando}
                      ...
                      <span style={{ fontSize: 11, color: '#5F5E5A' }}>pagÃ³ {c.veces} vez(ces) Â· asociar â†’</span>
                    </button>
                  ))}
                </>
              )}
              ...
              <div style={{ marginTop: 14, paddingTop: 12, borderTop: '0.5px solid #EDEBE4' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={() => asociarRut(asocId)} disabled={asocGuardando || !asocId.trim()}
                    ...
                    {asocGuardando ? 'Asociandoâ€¦' : 'Asociar'}
                  </button>
                </div>
              </div>
            </div>
