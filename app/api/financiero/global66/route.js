// RUTA: app/api/financiero/global66/route.js
// VERSION: v1 · 2026-08-16 · Endpoint de Global 66 (cuenta 1101-06).
//   GET   → lista de movimientos.
//   POST  → carga un lote de movimientos ya parseados (parseGlobal66). Dedup por id_transaccion:
...
const COLS = 'id, id_transaccion, fecha, tipo, glosa, tercero, debito, credito, moneda, costo_cambio, tipo_cambio, ccb, cuenta_contable, comentario, archivo, cargado_por, created_at, updated_at'

export async function GET() {
  const session = await getServerSession(authOptions)
  if (!session?.user?.email) return Response.json({ error: 'No autenticado' }, { status: 401 })
  const { data, error } = await admin.from('global66_movimientos').select(COLS).order('fecha', { ascending: false }).order('id', { ascending: false })
...
export async function POST(req) {
  ...
  const yaHay = new Set()
  if (ids.length) {
    const { data } = await admin.from('global66_movimientos').select('id_transaccion').in('id_transaccion', ids)
  ...
  const nuevos = movs.filter(m => { const id = txt(m.id_transaccion); return !id || !yaHay.has(id) }).map(m => ({
    id_transaccion: txt(m.id_transaccion), fecha: txt(m.fecha), tipo: txt(m.tipo), glosa: txt(m.glosa), tercero: txt(m.tercero),
    debito: numOf(m.debito), credito: numOf(m.credito), moneda: txt(m.moneda) || 'CLP',
    costo_cambio: numOf(m.costo_cambio), tipo_cambio: numOf(m.tipo_cambio), archivo: txt(body.archivo) || txt(m.archivo), cargado_por: email,
  }))
  let insertados = 0
  if (nuevos.length) {
    const { error } = await admin.from('global66_movimientos').insert(nuevos)
    if (error) return Response.json({ error: error.message }, { status: 500 })
...
export async function PATCH(req) {
  ...
  const { error } = await admin.from('global66_movimientos').update(patch).eq('id', body.id)
  if (error) return Response.json({ error: error.message }, { status: 500 })
