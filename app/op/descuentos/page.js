...
const EDIT_CAMPOS = [
  ...
  { k: 'repercutir_a', tipo: 'select', opts: null },   // REPERCUTIR_A (se pasa al render)
  ...
  { k: 'tipo', tipo: 'select', opts: null },           // TIPOS
...
function FichaDescuento({ descuento, caps, onClose, onGuardado }) {
  const [row, setRow] = useState(descuento);          // datos mostrados (se refresca al guardar)
  ...
  async function guardar() {
    setErr('');
    // validación ligera (el server valida de nuevo)
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 18px', borderBottom: `1px solid ${C.borde}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: C.azul }}>
              Descuento N° {row.num || '—'}
            </div>
            <div style={{ fontSize: 12, color: C.gris }}>
              {row.idadmon || '—'} · {row.tipo || ''} · {fmtFecha(row.fecha)}
        ...
        <div style={{ overflow: 'auto', padding: '14px 18px', flex: 1 }}>
          {modo === 'ver'
            ? SECCIONES_VER.map((sec) => (
                <div key={sec.titulo} style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: C.azul, textTransform: 'uppercase', letterSpacing: .4, marginBottom: 6, borderBottom: `1px solid ${C.azulClaro}`, paddingBottom: 3 }}>
                    {sec.titulo}
                  </div>
                  ...
                  return (
                    <div key={cfg.k} style={{ gridColumn: largo ? '1 / -1' : 'auto' }}>
                      <Campo label={LABELS[cfg.k] || cfg.k}>
                        {editorCampo(cfg, buf[cfg.k] ?? '', (val) => setBuf((b) => ({ ...b, [cfg.k]: val })), row)}
                      </Campo>
                    </div>
                  );
                })}
          ...
          <div style={{ marginTop: 18, paddingTop: 12, borderTop: `1px solid ${C.borde}` }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: C.azul, textTransform: 'uppercase', letterSpacing: .4, marginBottom: 8 }}>
              Bitácora
            </div>
...
function editorCampo(cfg, val, onChange, row) {
  const common = { value: val ?? '', onChange: (e) => onChange(e.target.value), style: inp };
  if (cfg.tipo === 'select') {
  ...
  if (cfg.tipo === 'mes') {
    ...
    if (actual && !opts.includes(actual)) opts.unshift(actual);   // no perder el mes actual
    return (
      <select value={val ?? ''} onChange={(e) => onChange(e.target.value)} style={inp}>
        <option value="">— elegir —</option>
