...
"        const label = p.dormitorios ? p.dormitorios+'D' : (ABREV[p.tipo && p.tipo.toUpperCase()] || (p.tipo && p.tipo.substring(0,3)) || '?')",
"        const bg = esVenta ? '#16a34a' : '#1a56db'",
"        const html = '<div style=\"background:'+bg+';color:#fff;padding:3px 7px;border-radius:12px;font-size:11px;font-weight:700;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,0.25)\">'+label+'</div>'",
"        const icon = L.divIcon({ className: '', html: html, iconAnchor:[20,10] })",
"        const precio = p.tipo_moneda==='UF' ? 'UF '+Number(p.valor).toLocaleString('es-CL') : '$'+Number(p.valor).toLocaleString('es-CL')",
"        const popup = '<div style=\"font-family:sans-serif;min-width:180px\"><b>'+p.codigo+' \u00b7 '+(p.tipo||'')+'</b><br/><span style=\"color:#555;font-size:12px\">'+(p.direccion||'')+', '+(p.comuna||'')+'</span><br/><b style=\"color:'+bg+'\">'+precio+'</b><br/><a href=\"/publicaciones/'+p.id+'\" style=\"color:#1a56db;font-size:11px\">Ver ficha \u2192</a></div>'",
"        L.marker([lat,lng],{icon}).addTo(_mapInstance).bindPopup(popup)",
"      })",
"    }",
"    init()",
